...
const FILTERS: [string, string, string][] = [["all", "All", "Tous"], ["couriers", "Couriers", "Chauffeurs"], ["senders", "Senders", "Expéditeurs"], ["unverified", "Unverified", "Non vérifiés"], ["founding", "Founding", "Fondateurs"], ["pending", "Pending", "En attente"]];
  ...
  return (
    <>
      ...
      <div className="toolbar">
        {filter !== "pending" && <input className="search" value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === "Enter" && load()} placeholder={t("🔍 name, email…", "🔍 nom, courriel…")} />}
        {FILTERS.map(([f, en, fr]) => <button key={f} className={"chip" + (filter === f ? " on" : "")} onClick={() => { setFilter(f); load(f, search); }}>{lang === "fr" ? fr : en}</button>)}
        {filter !== "pending" && <button className="btn ghost" style={{ marginLeft: "auto" }} onClick={nudgeAll}>🔔 {t("Nudge unverified", "Relancer les non vérifiés")}</button>}
      </div>
