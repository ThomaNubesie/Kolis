...
export default function Assistant() {
  ...
  const confirm = async (mi: number, p: Proposal) => {
    ...
    try {
      ...
      const summary = res.error ? `⚠︎ ${res.error}` : okSummary(p.name, res, lang);
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "6px 2px", display: "flex", flexDirection: "column", gap: 12 }}>
        {chat.length === 0 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 6 }}>
            {SUGGEST.map(([en, fr], i) => (
              <button key={i} onClick={() => send(lang === "fr" ? fr : en)} className="card" style={{ textAlign: "left", cursor: "pointer", border: "1px solid var(--border,#e7e2d6)", padding: "12px 14px", background: "#fff" }}>
                💬 {lang === "fr" ? fr : en}
              </button>
            ))}
          </div>
        ...
        {chat.map((m, mi) => (
          <div key={mi} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "88%" }}>
            <div style={{ padding: "11px 14px", borderRadius: 14, fontSize: 14.5, lineHeight: 1.55, whiteSpace: "pre-wrap",
            ...
            {(m.proposals || []).map((p) => {
              ...
              return (
                <div key={p.id} style={{ marginTop: 8, background: "#FBF3F7", border: "1px solid #F3D6E4", borderRadius: 12, padding: "12px 14px" }}>
                  <div style={{ fontWeight: 800, fontSize: 13, color: "#B21561", marginBottom: 6 }}>⚑ {t("Proposed action", "Action proposée")}: {label}</div>
                  <div style={{ fontSize: 12.5, color: "#6b1440", lineHeight: 1.6 }}>
                    {Object.entries(p.input || {}).filter(([, v]) => v).map(([k, v]) => (
                      <div key={k}><b>{k}:</b> {String(v).slice(0, 180)}</div>
                    ))}
                  </div>
                  {done ? (
                    <div style={{ marginTop: 10, fontWeight: 700, fontSize: 13, color: done.startsWith("⚠") ? "#b91c1c" : "#178a5e" }}>{done}</div>
                  ) : (
                    <div className="row" style={{ gap: 8, marginTop: 11 }}>
                      <button onClick={() => confirm(mi, p)} disabled={confirming === p.id}
                        style={{ background: "#16A34A", color: "#fff", border: "none", borderRadius: 9, padding: "9px 16px", fontWeight: 700, fontSize: 13, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 6 }}>
                        {confirming === p.id ? <Loader2 size={14} className="spin" /> : <Check size={15} />}{t("Confirm & run", "Confirmer et exécuter")}
                      </button>
                      <button onClick={() => dismiss(mi, p)} style={{ background: "none", border: "1px solid #e2ddd0", borderRadius: 9, padding: "9px 14px", fontWeight: 600, fontSize: 13, cursor: "pointer", color: "#6B6675", display: "inline-flex", alignItems: "center", gap: 6 }}>
...
function okSummary(name: string, res: any, lang: string): string {
  const fr = lang === "fr";
  const r = res.result ?? res;
  if (name === "create_shipment") { const code = r?.code || r?.parcel_code || r?.tracking_code || (typeof r === "string" ? r : ""); return fr ? `✓ Envoi créé${code ? " · " + code : ""}` : `✓ Shipment created${code ? " · " + code : ""}`; }
