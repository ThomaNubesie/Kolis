...
//
// POST { kind: "meeting" | "booking", id, reminder?: true }
...
function emailHtml(o: { kind: string; title: string; where: string; whenEn: string; whenFr: string; mins: number; link: string; note?: string | null; reminder?: boolean }) {
  ...
  const headFr = o.reminder ? "Cela commence bientôt." : (o.kind === "booking" ? "Votre rendez-vous est confirmé." : "Vous êtes convoqué(e).");
  const headEn = o.reminder ? "Starting soon." : (o.kind === "booking" ? "Your meeting is confirmed." : "You are called to a meeting.");
...
function buildIcs(o: { id: string; title: string; where: string; startsAt: string; mins: number; link: string; note?: string | null; cancelled?: boolean }) {
...
Deno.serve(async (req) => {
  ...
  try {
    const { kind, id, reminder } = await req.json().catch(() => ({} as any));
    ...
    if (kind === "meeting") {
      ...
      if (!m || m.status !== "scheduled") return json({ ok: false, error: "not_found" }, 404);
      ...
      people = (rows ?? []).filter((p: any) => p.user_id !== m.created_by);
    } else {
      ...
      if (!b || b.status !== "booked") return json({ ok: false, error: "not_found" }, 404);
    ...
    const html = emailHtml({ kind, title, where, whenEn: w.en, whenFr: w.fr, mins, link, note, reminder });
    const ics = buildIcs({ id, title, where, startsAt, mins, link, note });
