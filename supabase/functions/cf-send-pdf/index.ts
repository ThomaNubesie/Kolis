...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { form_id, filename, pdf_base64, recipients, message } = await req.json().catch(() => ({} as any));
    if (!form_id || !pdf_base64) return json({ error: "missing_args" }, 400);
    ...
    const formName = form.name ?? "Quorly form";
    ...
    const r = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${RESEND}`, "Content-Type": "application/json" },
      body: JSON.stringify({ from: "Quorly <noreply@loadq.ca>", to, subject: `“${formName}” — Quorly export`,
