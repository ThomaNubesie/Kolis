  ...
  try {
    ...
    if (action === "remind") {
      ...
      const { data: due } = await admin.from("loadq_pickup_requests")
        .select("id,contact_phone,pickup_address,drop_zone_id,destination_region,total_cents,pay_ref,remind_count,reminded_at")
        .eq("status", "quoted").is("paid_at", null).not("contact_phone", "is", null)
        .lt("created_at", cutoff).lt("remind_count", 2);
      const zoneIds = [...new Set((due ?? []).map((r: any) => r.drop_zone_id).filter(Boolean))];
      const { data: zrows } = zoneIds.length
        ? await admin.from("zones").select("id,name,address").in("id", zoneIds)
        : { data: [] as any[] };
      const zoneOf = new Map((zrows ?? []).map((z: any) => [z.id, z]));
      ...
      for (const r of due ?? []) {
        ...
        // The itinerary here is the PICKUP leg — the rider's address to the loading point.
        // It used to read destination_region, so a rider collected in Gatineau and dropped at
        // Universal Grocery in Ottawa was told his trip was "vers Montreal". Montreal is where
        // he goes next, in another car, on a fare he has not bought. Naming it here described a
        // journey nobody was selling him and hid the one he was being asked to pay for.
        const z = zoneOf.get(r.drop_zone_id as string);
        const dropoff = z ? (z.address ? `${z.name} — ${z.address}` : z.name) : "le point d'embarquement";
        const onward = (r.destination_region || "").replace(/^\w/, (c: string) => c.toUpperCase());
        const body =
          `Rappel — votre ramassage LoadQ (${amt}) attend votre paiement.\n`
          + `De : ${r.pickup_address}\n`
          + `À : ${dropoff}\n`
          + (onward ? `Puis départ vers ${onward}.\n` : "")
          + `Virement Interac à ${INTERAC_TO}, code ${r.pay_ref} dans le message. Votre chauffeur part dès réception.\n\n`
          + `Reminder — your LoadQ pickup (${amt}) is awaiting payment.\n`
          + `From: ${r.pickup_address}\n`
          + `To: ${dropoff}\n`
          + (onward ? `Then onward to ${onward}.\n` : "")
          + `Interac e-Transfer to ${INTERAC_TO} with code ${r.pay_ref}. Your driver is dispatched once it clears.`;
        if (SID && TK && FR) {
