// Resend webhook receiver for the Quorly outreach campaign.
// Logs delivered/opened/clicked/bounced/complained into quorly_outreach_events and
// updates the matching quorly_outreach recipient (opened_at / clicked_at / bounced_at;
// a click or bounce drops them out of the follow-up cadence).
...
const WHSEC = Deno.env.get("QUORLY_RESEND_WEBHOOK_SECRET") || ""; // "whsec_<base64>"
...
Deno.serve(async (req) => {
  ...
  const type = String(ev.type || "").replace(/^email\./, "");
  const data = ev.data || {};
  ...
  if (email) {
    ...
    if (type === "complained") { patch.status = "stopped"; patch.next_due_at = null; }
    if (Object.keys(patch).length) await admin.from("quorly_outreach").update(patch).eq("email", email);
  }
